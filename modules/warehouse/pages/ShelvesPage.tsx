...
import type { WarehouseLocation, Warehouse } from "@/modules/warehouse/types";
...
const readPayloadString = (record: WarehouseLocation, key: string, fallback = ""): string => {
...
const calculateTotalBoxes = (record: WarehouseLocation): number => {
...
export function ShelvesPage() {
  ...
  const highlightWarehouseId = searchParams.get("highlight_warehouse_id");
  ...
  const shelvesQuery = useQuery({
    ...
    queryFn: () =>
      warehouseApi.listLocations({
        ...
        warehouse_id: tableQuery.warehouse_id,
      }).then(res => res.data),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload: Partial<WarehouseLocation> = {
        ...
        warehouse_id: form.parent_id ? Number(form.parent_id) : undefined,
        type: 'shelf',
      ...
      if (form.id) {
        return warehouseApi.updateLocation(form.id, payload);
      }

      return warehouseApi.createLocation(payload);
    },
    onSuccess: () => {
      toast.success(form.id ? t("inventory.common.saved", "Shelf updated.") : t("inventory.common.saved", "Shelf created."));
      queryClient.invalidateQueries({ queryKey: ["warehouse", "shelves"] });
      setSelectedRowIds({});
      closeModal();
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => warehouseApi.deleteLocation(id),
    onSuccess: () => {
  ...
  const openEdit = React.useCallback((shelf: WarehouseLocation) => {
    setForm({
      id: shelf.id,
      name: shelf.name ?? "",
      code: shelf.code ?? "",
      parent_id: shelf.warehouse_id ? String(shelf.warehouse_id) : "",
  ...
  const exportUrl = React.useMemo(() => {
    ...
    return `/api/v1/warehouse/locations/export?type=shelf&${params.toString()}`;
  ...
  const columns = React.useMemo<ColumnDef<WarehouseLocation>[]>(
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          const isHighlighted = highlightWarehouseId && String(shelf.warehouse_id) === highlightWarehouseId;
          return (
            <div>
            ...
            <div className="flex justify-start gap-2">
              {addProductId ? (
                <Button
                  ...
                  onClick={() =>
                    router.push(`/dashboard/warehouse/locations/boxes?highlight_parent_id=${shelf.id}&add_product_id=${addProductId}`)
                  }
                >
    ...
    [deleteMutation, openEdit, addProductId, router, t, highlightWarehouseId]
  );
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-muted-foreground">
                {t("inventory.shelves.pick_shelf", "Pick a shelf to store")} <span className="font-bold underline text-primary">{(prodQuery.data as any)?.product?.name ?? "Loading..."}</span>
              </p>
            </div>
          </div>
          <Button 
      ...
      <DataTable
        ...
        totalEntries={shelvesQuery.data?.meta?.total ?? shelvesQuery.data?.total ?? 0}
