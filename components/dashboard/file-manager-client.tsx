...
const matchesAcceptedFileTypes = (
  ...
  return acceptedFileTypes
    ...
    .some((type) => {
      ...
      if (type.endsWith("/*"))
        return normalizedMimeType.startsWith(type.slice(0, -1));
  ...
  return (
    <>
      <div
        ref={triggerRef}
        onClick={toggleMenu}
        className="relative inline-block cursor-pointer z-50 overflow-visible"
      >
...
const MenuItem = ({ icon, label, onClick, danger }: MenuItemProps) => (
  <button
    ...
    className={cn(
      "flex items-center justify-start gap-3 w-full px-4 py-2.5 text-sm text-left hover:bg-muted/60 transition-colors font-medium",
...
const extractFileIdFromServeUrl = (
...
const AuthImage = ({
  ...
  const isTenantUrl =
    src?.includes("/api/v1/files/") && src?.includes("/serve");
...
const getDownloadNameFromDisposition = (
...
export function FileManagerClient({
  ...
  const canRead =
    access?.canRead ?? hasAnyPermission(["view_storage", "manage_storage"]);
  ...
  const [checkoutModuleSlug, setCheckoutModuleSlug] = React.useState<
    "image_editor" | "video_player" | "audio_player" | null
  >(null);
  ...
  const lockedModule = checkoutModuleSlug
    ? (subscriptionData?.data?.module_subscriptions?.catalog_modules?.find(
        (module: FileManagerCatalogModule) =>
          module.slug === checkoutModuleSlug,
      ) ?? null)
    : null;
  ...
  const [activeFilter, setActiveFilter] = React.useState<
    "all" | "favorites" | "trash" | "recent"
  >("all");
  const [activeTypeFilter, setActiveTypeFilter] = React.useState<
    | "image"
    | "video"
    | "document"
    | "audio"
    ...
    | null
  >(null);
  const [activePlaylistId, setActivePlaylistId] = React.useState<number | null>(
    null,
  );
  ...
  const [itemToAddToPlaylist, setItemToAddToPlaylist] = React.useState<{
    id: number;
    type: "file" | "folder";
  } | null>(null);
      ...
      try {
        ...
        const pollPreparation = async (): Promise<string | null> => {
          ...
          try {
            ...
            throw new Error(
              "The media server is unavailable. Please wait a moment and try the download again.",
            );
          }
          ...
          if (!res.ok) {
            throw new Error(
              data?.message ||
                `Download preparation failed (status ${res.status}).`,
            );
          }
          ...
          if (data.status === "failed") {
            throw new Error(
              data.message || "The protected video could not be prepared.",
            );
          }

          if (data.status === "processing" && attempt < 120) {
            // Max 6 minutes
            if (!didShowPreparingToast) {
              didShowPreparingToast = true;
              toast.loading("Applying the protected video watermark...", {
                id: `download-${fileId}`,
              });
            }
            ...
            if (typeof data.progress === "number") {
              ...
              toast.loading(`Preparing download: ${data.progress}%`, {
                id: `download-${fileId}`,
              });
            }
  ...
  const [selectedFile, setSelectedFile] =
    React.useState<FileManagerFile | null>(null);
  ...
  const [customThumbnail, setCustomThumbnail] = React.useState<File | null>(
    null,
  );
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (currentFolderId)
        params.append("folder_id", currentFolderId.toString());
      if (activePlaylistId)
        params.append("playlist_id", activePlaylistId.toString());
  ...
  React.useEffect(() => {
    ...
    const didMediaStateChange =
      ...
      (currentMedia.subtitles?.length ?? 0) !==
        (nextMedia.subtitles?.length ?? 0);
  ...
  const moveItemsMut = useMutation({
    mutationFn: async () => {
      const destId =
        moveTargetFolder === "root" ? null : parseInt(moveTargetFolder);
  ...
  const emptyTrashMut = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${getBackendApiRoot()}/files/trash/empty`, {
        method: "POST",
        headers: getAuthHeaders(),
      });
  ...
  const forceDeleteItemsMut = useMutation({
    mutationFn: async (items: FileManagerItemRef[]) => {
      const res = await fetch(
        `${getBackendApiRoot()}/files/trash/force-delete`,
        {
          method: "POST",
          headers: getAuthHeaders({ "Content-Type": "application/json" }),
          body: JSON.stringify({ items }),
  ...
  const shareLinkMut = useMutation({
    ...
    onSuccess: (data) => {
      navigator.clipboard.writeText(
        data.link || "https://hive.os/share/mock-link-8x92a",
      );
  ...
  const saveEditedImageMut = useMutation({
    mutationFn: async ({
      ...
      if (!res.ok) {
        ...
        if (res.status === 402 && payload?.module?.slug === "image_editor") {
          throw Object.assign(
            new Error(
              payload?.message || "Image Editor requires a subscription.",
            ),
            {
              module: "image_editor",
            },
    ...
    onError: (error: unknown) => {
      if (
        ...
        "module" in error &&
        (error as ModuleCheckoutError).module === "image_editor"
      ) {
  ...
  const toggleFavoriteMut = useMutation({
    mutationFn: async ({
      ...
      const res = await fetch(
        `${getBackendApiRoot()}/files/${type}/${id}/favorite`,
        {
          method: "POST",
          headers: getAuthHeaders(),
        },
  ...
  const addToPlaylistMut = useMutation({
    mutationFn: async ({
      ...
      const res = await fetch(
        `${getBackendApiRoot()}/playlists/${playlistId}/add`,
        {
          method: "POST",
          headers: getAuthHeaders({ "Content-Type": "application/json" }),
  ...
  const uploadFileMut = useMutation({
    mutationFn: async (): Promise<{ queued: boolean }> => {
      ...
      if (uploadTargetFolder !== "root")
        offlineFields.folder_id = uploadTargetFolder;
      ...
      for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
        ...
        if (uploadTargetFolder !== "root")
          formData.append("folder_id", uploadTargetFolder);
        if (uploadBaseName) formData.append("base_name", uploadBaseName);
        if (chunkIndex === totalChunks - 1 && customThumbnail)
          formData.append("custom_thumbnail", customThumbnail);
        ...
        if (!res.ok) {
          ...
          try {
            ...
            if (res.status === 402 && data.module) {
              throw Object.assign(
                new Error(data.message || "Subscription required."),
                { module: data.module },
              );
            }
          } catch (e) {}
    ...
    onSuccess: (result) => {
      if (result?.queued) {
        toast.success(
          "Upload queued — it'll sync automatically when you're back online",
          { duration: 6000 },
        );
      } else {
        queryClient.invalidateQueries({ queryKey: ["files"] });
    ...
    onError: (err: unknown) => {
      toast.error(getErrorMessage(err, "Upload failed"), { duration: 8000 });
      setUploadProgress(0);
    },
  });
  ...
  const uploadSubtitleMut = useMutation({
    mutationFn: async ({
      ...
      const res = await fetch(
        `${getBackendApiRoot()}/files/upload-subtitle/${fileId}`,
        {
          method: "POST",
          headers: getAuthHeaders(),
          body: formData,
      ...
      if (!res.ok) {
        ...
        if (res.status === 402 && payload?.module?.slug === "video_player") {
          throw Object.assign(
            new Error(
              payload?.message || "Video Player requires a subscription.",
            ),
            {
              module: "video_player",
            },
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["files"] });
      toast.success("Subtitle track added successfully!");
    ...
    onError: (error: unknown) => {
      if (
        error instanceof Error &&
        "module" in error &&
        (error as ModuleCheckoutError).module === "video_player"
      ) {
  ...
  const deleteSubtitleMut = useMutation({
    mutationFn: async (subtitleUuid: string) => {
      const res = await fetch(
        `${getBackendApiRoot()}/files/subtitle/${subtitleUuid}`,
        {
          method: "DELETE",
          headers: getAuthHeaders(),
        },
      ...
      if (selectedFile) {
        setSelectedFile({
          ...
          media_details: {
            ...
            subtitles: (selectedFile.media_details?.subtitles || []).filter(
              (s) => s.uuid !== deletedUuid,
            ),
          },
        });
      }
  ...
  const handleCreateFolder = (e: React.FormEvent) => {
    e.preventDefault();
    if (!canManage || !folderName.trim()) return;
  ...
  const submitSubtitle = () => {
    if (!canManage || !subtitleFile || !selectedFile) return;
    uploadSubtitleMut.mutate({
  ...
  // 🚀 Sorting Execution
  const getItemMediaDetails = (
    item: FileManagerSortable,
  ): FileManagerMediaDetails | undefined =>
  ...
  const sortData = (a: FileManagerSortable, b: FileManagerSortable) => {
    ...
    if (sortBy === "name") {
      const nameA =
        ("name" in a ? a.name : "") || mediaA?.title || mediaA?.name || "";
      const nameB =
        ("name" in b ? b.name : "") || mediaB?.title || mediaB?.name || "";
  ...
  if (searchQuery) {
    ...
    allFiles = allFiles.filter((f) =>
      (f.media_details?.title || f.media_details?.name || "")
        .toLowerCase()
        .includes(q),
    );
  }
  ...
  const displayedFiles = allFiles.filter((file) => {
    ...
    if (activeTypeFilter === "document")
      return /(document|pdf|text|msword|excel|spreadsheet|powerpoint|presentation|csv)/i.test(
        mime,
      );
    if (activeTypeFilter === "model")
      return (
        mime.startsWith("model/") ||
        (mime === "application/octet-stream" &&
          (file.media_details?.name?.toLowerCase().endsWith(".glb") ||
            file.media_details?.name?.toLowerCase().endsWith(".gltf")))
      );
    if (activeTypeFilter === "archive")
      return /(zip|rar|tar|gzip|7z|compressed)/i.test(mime);
    return (
      !mime.startsWith("image/") &&
      !mime.startsWith("video/") &&
      !mime.startsWith("audio/") &&
      !/(document|pdf|text|msword|excel|spreadsheet|powerpoint|presentation|csv|zip|rar|tar|gzip|7z|compressed)/i.test(
        mime,
      ) &&
      !mime.startsWith("model/")
    );
  ...
  allFiles.forEach((file) => {
    ...
    if (mime.startsWith("image/")) {
      categoryStats.image.size += size;
      categoryStats.image.count++;
    } else if (mime.startsWith("video/")) {
      categoryStats.video.size += size;
      categoryStats.video.count++;
    } else if (mime.startsWith("audio/")) {
      categoryStats.audio.size += size;
      categoryStats.audio.count++;
    } else if (
      /(document|pdf|text|msword|excel|spreadsheet|powerpoint|presentation|csv)/i.test(
        mime,
      )
    ) {
      categoryStats.document.size += size;
      categoryStats.document.count++;
    } else if (
      mime.startsWith("model/") ||
      (mime === "application/octet-stream" &&
        (file.media_details?.name?.toLowerCase().endsWith(".glb") ||
      ...
      categoryStats.model.size += size;
      categoryStats.model.count++;
    } else if (/(zip|rar|tar|gzip|7z|compressed)/i.test(mime)) {
      categoryStats.archive.size += size;
      categoryStats.archive.count++;
    } else {
      categoryStats.other.size += size;
      ...
      <div className="flex min-h-[60vh] items-center justify-center rounded-[2rem] border border-border/50 bg-card/40 p-8 text-center">
        <div className="max-w-md space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            Your current role does not have permission to view storage files.
          </p>
        </div>
      </div>
    );
  ...
  const renderFilePreview = (file: FileManagerFile) => {
      ...
      return (
        <div className="w-full h-full flex flex-col rounded-2xl overflow-hidden min-h-[50vh]">
          <ImageViewer
            src={safeUrl}
            fetchUrl={
              media.url?.includes("/api/v1/files/")
                ? media.url
                : `${getBackendApiRoot()}/files/${file.id}/download`
            }
            alt={mediaTitle}
            onSaveEdited={
              canManage && hasImageEditor
                ? (f: File) =>
                    saveEditedImageMut.mutate({ file: f, originalId: file.id })
                : undefined
            }
            onUpgradeRequested={
              canManage && !hasImageEditor
    ...
    if (mime.startsWith("video/")) {
      const videoFiles = fileItems.filter((f) =>
        f.media_details?.mime_type?.startsWith("video/"),
      );
      const currentIndex = videoFiles.findIndex((f) => f.id === file.id);
      const handleNext = () => {
        if (currentIndex < videoFiles.length - 1)
          setSelectedFile(videoFiles[currentIndex + 1]);
      };
      const handlePrev = () => {
        if (currentIndex > 0) setSelectedFile(videoFiles[currentIndex - 1]);
      };
      const formattedSubtitles = (media.subtitles || [])
        .map((sub: FileManagerSubtitle) => ({
          ...sub,
          src: sub.uuid
            ? `${getBackendApiRoot()}/files/subtitle/${sub.uuid}`
            : sub.src || "",
        ...
        return (
          <div className="flex min-h-[400px] w-full flex-col items-center justify-center rounded-2xl border border-border/50 bg-card/40 p-8 text-center shadow-inner">
            ...
            <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
              This tenant can keep the file in storage, but playback and
              subtitles require the `video_player` module.
            </p>
            {canManage ? (
              <Button
                onClick={() => setCheckoutModuleSlug("video_player")}
        ...
        <div className="flex flex-col items-center justify-center bg-black rounded-2xl h-full min-h-[400px] border border-border/50 overflow-hidden shadow-inner w-full relative">
          ...
          {hlsUrl || safeUrl ? (
            <VideoPlayer
              ...
              onNext={
                currentIndex < videoFiles.length - 1 ? handleNext : undefined
              }
    ...
    if (mime.startsWith("audio/")) {
        ...
        return (
          <div className="flex h-full min-h-[350px] w-full flex-col items-center justify-center rounded-2xl border border-border/50 bg-muted/10 p-8 text-center">
            ...
            <p className="text-lg font-black text-foreground">
              Audio Player Subscription Required
            </p>
            <p className="mt-2 max-w-md text-sm text-muted-foreground">
              This tenant can store audio files, but playback and background
              queues require the `audio_player` module.
            </p>
            <Button
              onClick={() => setCheckoutModuleSlug("audio_player")}
              className="mt-6 rounded-xl px-6 font-semibold"
            >
              Unlock Audio Player
            </Button>
      ...
      const audioFiles = fileItems.filter((f) =>
        f.media_details?.mime_type?.startsWith("audio/"),
      );
      const currentIndex = audioFiles.findIndex((f) => f.id === file.id);
      const handleNext = () => {
        if (currentIndex < audioFiles.length - 1)
          setSelectedFile(audioFiles[currentIndex + 1]);
      };
      const handlePrev = () => {
        if (currentIndex > 0) setSelectedFile(audioFiles[currentIndex - 1]);
      };
      const currentPlaylist = audioFiles.map((f) => ({
        id: f.id,
        src: getStorageUrl(f.media_details?.url),
        title:
          f.media_details?.title || f.media_details?.name || "Unknown Track",
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl h-full min-h-[350px] border border-border/50 shadow-inner w-full overflow-hidden relative bg-muted/10">
          <div className="absolute top-4 right-4 z-[60]">
            <Button
              variant="outline"
              size="icon"
              className="h-8 w-8 bg-background/80 backdrop-blur border-border/50 shadow-sm hover:bg-emerald-500 hover:border-emerald-500 hover:text-white transition-all rounded-xl group"
              onClick={() => {
                playTrack(
                  {
                    id: file.id,
                    src: streamSrc,
                    title: mediaTitle,
                    artist: media.artist || "HIVE.OS Audio",
                    coverArt: getStorageUrl(media.thumbnail),
                    isFavorite: file.is_favorite,
                    downloadUrl: `${getBackendApiRoot()}/files/${file.id}/download`,
                  },
                  currentPlaylist,
                );
                setSelectedFile(null);
          ...
          <AudioPlayer
            ...
            downloadUrl={`${getBackendApiRoot()}/files/${file.id}/download`}
            onNext={
              currentIndex < audioFiles.length - 1 ? handleNext : undefined
            }
            onPrevious={currentIndex > 0 ? handlePrev : undefined}
            className="shadow-2xl scale-105 z-10 w-[80%] max-w-[450px]"
            autoPlay={false}
          />
        </div>
      );
    ...
    if (
      mime.startsWith("model/") ||
      (mime === "application/octet-stream" &&
        (media.name?.toLowerCase().endsWith(".glb") ||
          media.name?.toLowerCase().endsWith(".gltf")))
    ) {
      return (
        <div className="h-full w-full min-h-[75vh] rounded-2xl overflow-hidden border border-border/50">
          <Model3DViewer src={getStreamUrl(safeUrl)} alt={mediaTitle} />
        </div>
      );
    }
    if (
      /(document|msword|excel|spreadsheet|powerpoint|presentation|csv)/i.test(
        mime,
      )
    )
      return (
    ...
    return (
      <div className="flex flex-col items-center justify-center bg-muted/20 rounded-2xl h-full min-h-[40vh] border border-dashed border-border/50 w-full">
  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-64 shrink-0 flex flex-col gap-4 lg:gap-6 bg-card/40 border border-border/50 rounded-[2rem] p-4 lg:p-5 backdrop-blur-xl overflow-hidden">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Organize files for {tenantName || "Central"}
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9 bg-background/50 border-border/50 h-10 rounded-xl text-sm"
          />
        </div>
        ...
        <nav className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 custom-scrollbar shrink-0">
          <button
            ...
            className={cn(
              "whitespace-nowrap flex-shrink-0 lg:w-full flex items-center px-4 lg:px-3 py-2.5 rounded-xl text-sm font-bold transition-all",
              activeFilter === "all" && !activePlaylistId
          ...
          <button
            ...
            className={cn(
              "whitespace-nowrap flex-shrink-0 lg:w-full flex items-center gap-3 px-4 lg:px-3 py-2.5 rounded-xl text-sm font-bold transition-all",
          ...
          <div className="flex lg:flex-col gap-1">
            {playlists.map((pl) => (
              <div
                key={pl.id}
                className="group/pl relative w-full flex-shrink-0 lg:w-full"
              >
                <div
                  role="button"
                  tabIndex={0}
                  ...
                  className={cn(
                    "whitespace-nowrap cursor-pointer flex-shrink-0 lg:w-full flex items-center justify-between px-4 lg:px-3 py-2 rounded-xl text-xs font-bold transition-all",
        ...
        {canManage && selectedItems.length > 0 ? (
          <div className="flex flex-col sm:flex-row items-center justify-between bg-emerald-500/10 border border-emerald-500/30 p-4 sm:px-6 rounded-[2rem] lg:rounded-[2.5rem] backdrop-blur-xl shadow-lg animate-in slide-in-from-top-4 gap-4 shrink-0">
            <div className="flex items-center gap-4">
              <Badge className="bg-emerald-500 text-emerald-950 font-black px-3 py-1 text-sm rounded-lg">
            ...
            <div className="flex gap-2 w-full sm:w-auto overflow-x-auto custom-scrollbar pb-1 sm:pb-0">
              <Button
                ...
                className="text-emerald-500 hover:bg-emerald-500/20 whitespace-nowrap"
              >
              ...
              {activeFilter === "trash" ? (
                <>
                  <Button
                    size="sm"
                    variant="outline"
                    className="border-emerald-500/50 text-emerald-500 hover:bg-emerald-500/10 rounded-xl whitespace-nowrap"
                    onClick={() => restoreItemsMut.mutate(selectedItems)}
                  >
                    {restoreItemsMut.isPending ? (
                  ...
                  <Button
                    size="sm"
                    className="bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold whitespace-nowrap"
                    onClick={() => forceDeleteItemsMut.mutate(selectedItems)}
                  >
                    {forceDeleteItemsMut.isPending ? (
                      <Loader2 className="h-4 w-4 animate-spin mr-2" />
                    ) : (
                ...
                <>
                  <Button
                    ...
                    variant="outline"
                    className="border-emerald-500/50 text-emerald-500 hover:bg-emerald-500/10 rounded-xl whitespace-nowrap"
                    onClick={() => openMoveModal(selectedItems)}
                  >
                  ...
                  <Button
                    ...
                    className="bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold whitespace-nowrap"
                    onClick={() =>
                      selectedItems.forEach((item) => deleteMut.mutate(item))
                    }
                  >
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-card/40 border border-border/50 p-4 lg:p-5 px-6 rounded-[2rem] lg:rounded-[2.5rem] backdrop-blur-xl shrink-0 shadow-sm">
            <div>
              <h1 className="text-xl lg:text-2xl font-black tracking-tight">
                {activeFilter === "trash" ? "Recycle Bin" : "Cloud Drive"}
              </h1>
              <p className="text-xs lg:text-sm text-muted-foreground mt-1 flex items-center gap-2">
                ...
                {activeFilter === "trash"
                  ...
                  : activeTypeFilter
                    ? `Filtering by ${activeTypeFilter}s`
                    : "Quick access to your folders and files"}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2 lg:gap-3 w-full xl:w-auto">
              ...
              <SimpleMenu
                trigger={
                  <Button
                    variant="outline"
                    className="h-10 rounded-xl border-border/50 bg-background/50 hover:bg-muted text-xs font-bold px-4"
                  >
                    <SortAsc className="h-4 w-4 mr-2" /> Sort By
                  </Button>
                }
              >
                <MenuItem
              ...
              {canManage &&
                (activeFilter === "trash" ? (
                  <Button
                    ...
                    disabled={folders.length === 0 && allFiles.length === 0}
                    className="rounded-xl h-10 px-4 font-bold bg-red-500 text-white hover:bg-red-600 shadow-sm flex-1 sm:flex-none"
                  >
                  ...
                  <>
                    <Button
                      ...
                      className="rounded-xl h-10 px-4 font-bold border-border/50 bg-background/50 hover:bg-muted shadow-sm flex-1 sm:flex-none"
                    >
                    ...
                    <Button
                      onClick={() => {
                        setUploadTargetFolder(
                          currentFolderId ? currentFolderId.toString() : "root",
                        );
                        setIsUploadOpen(true);
                        console.log("Upload modal opened");
                      }}
                      className="rounded-xl h-10 px-4 font-bold bg-emerald-500 text-emerald-950 hover:bg-emerald-400 shadow-sm flex-1 sm:flex-none"
                    >
        ...
        {activeFilter !== "trash" && (
          <div className="shrink-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
              <MetricCard
                isActive={activeTypeFilter === "image"}
                onClick={() =>
                  setActiveTypeFilter(
                    activeTypeFilter === "image" ? null : "image",
                  )
                }
                icon={<ImageIcon className="h-4 w-4 text-purple-400" />}
                title="Images"
                size={categoryStats.image.size}
                count={categoryStats.image.count}
                color="bg-purple-500/10 border-purple-500/20"
              />
              <MetricCard
                isActive={activeTypeFilter === "video"}
                onClick={() =>
                  setActiveTypeFilter(
                    activeTypeFilter === "video" ? null : "video",
                  )
                }
                icon={<Video className="h-4 w-4 text-blue-400" />}
                title="Videos"
                size={categoryStats.video.size}
                count={categoryStats.video.count}
                color="bg-blue-500/10 border-blue-500/20"
              />
              <MetricCard
                isActive={activeTypeFilter === "document"}
                onClick={() =>
                  setActiveTypeFilter(
                    activeTypeFilter === "document" ? null : "document",
                  )
                }
                icon={<FileText className="h-4 w-4 text-amber-400" />}
                title="Docs"
                size={categoryStats.document.size}
                count={categoryStats.document.count}
                color="bg-amber-500/10 border-amber-500/20"
              />
              <MetricCard
                isActive={activeTypeFilter === "audio"}
                onClick={() =>
                  setActiveTypeFilter(
                    activeTypeFilter === "audio" ? null : "audio",
                  )
                }
                icon={<Music className="h-4 w-4 text-pink-400" />}
                title="Audio"
                size={categoryStats.audio.size}
                count={categoryStats.audio.count}
                color="bg-pink-500/10 border-pink-500/20"
              />
              <MetricCard
                isActive={activeTypeFilter === "model"}
                onClick={() =>
                  setActiveTypeFilter(
                    activeTypeFilter === "model" ? null : "model",
                  )
                }
                icon={<Box className="h-4 w-4 text-cyan-400" />}
                title="3D Models"
                size={categoryStats.model.size}
                count={categoryStats.model.count}
                color="bg-cyan-500/10 border-cyan-500/20"
              />
              <MetricCard
                isActive={activeTypeFilter === "archive"}
                onClick={() =>
                  setActiveTypeFilter(
                    activeTypeFilter === "archive" ? null : "archive",
                  )
                }
                icon={<Archive className="h-4 w-4 text-orange-400" />}
                title="Archives"
                size={categoryStats.archive.size}
                count={categoryStats.archive.count}
                color="bg-orange-500/10 border-orange-500/20"
              />
              <MetricCard
                isActive={activeTypeFilter === "other"}
                onClick={() =>
                  setActiveTypeFilter(
                    activeTypeFilter === "other" ? null : "other",
                  )
                }
                icon={<FileIcon className="h-4 w-4 text-gray-400" />}
                title="Others"
                size={categoryStats.other.size}
                count={categoryStats.other.count}
              ...
              {activeFilter !== "recent" &&
                ...
                folders.length > 0 && (
                  <section
                    className={cn(
                      viewMode === "list" &&
                        "bg-card/30 border border-border/50 rounded-[2rem] p-4",
                    )}
                  >
                    {viewMode === "grid" && (
                      <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground/60 mb-4 px-2">
                        Folders
                      </h3>
                    )}
                    <div
                      ...
                      {folders.map((folder) => {
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              "bg-card/40 border rounded-2xl cursor-pointer transition-all relative group flex",
                              viewMode === "grid"
                                ? "flex-col p-4 hover:shadow-md hover:-translate-y-0.5"
                                : "flex-row items-center gap-4 p-2 pr-4 hover:bg-muted/40",
                              isSelected
                          ...
                          >
                            ...
                            {canManage && (
                              ...
                              >
                                <input
                                  ...
                                  className="h-4 w-4 rounded accent-emerald-500 cursor-pointer shadow-md"
                                />
                              </div>
                            )}
                            ...
                            <div
                              className={cn(
                                "rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 shrink-0",
                                viewMode === "grid"
                                  ? "h-10 w-10 mb-3 mx-auto relative z-10"
                                  : "h-10 w-10 ml-8",
                              )}
                            >
                            ...
                            <h4 className="font-bold text-sm truncate flex-1 text-center sm:text-left">
                              {folder.name}
                            </h4>
                            {viewMode === "list" && (
                              <span className="text-xs text-muted-foreground font-mono w-32 hidden md:block text-right">
                                {new Date(
                                  folder.created_at,
                                ).toLocaleDateString()}
                              </span>
                                ...
                                <SimpleMenu
                                  trigger={
                                    <Button
                                      variant="ghost"
                                      size="icon"
                                      className="h-8 w-8 text-muted-foreground hover:bg-background/80 lg:opacity-0 group-hover:opacity-100 transition-opacity"
                                    >
                  ...
                  <div className="flex justify-between items-center mb-4 px-1">
                    ...
                    {displayedFiles.length > 5 && (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="text-emerald-500 hover:text-emerald-600 hover:bg-emerald-500/10 h-8 font-bold rounded-lg px-3"
                      ...
                      {visibleFiles.map((file) => {
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              "bg-card border rounded-2xl group cursor-pointer transition-all relative flex min-w-0",
                              viewMode === "grid"
                                ? "flex-col p-3 hover:shadow-md hover:-translate-y-0.5"
                                : "flex-row items-center gap-4 p-2 pr-4 hover:bg-muted/40",
                              isSelected
                                ? "border-emerald-500 ring-1 ring-emerald-500/50 bg-emerald-500/5"
                          ...
                          >
                            ...
                            <div
                              className={cn(
                                "bg-muted/30 rounded-xl flex items-center justify-center overflow-hidden border border-border/50 relative z-10 shrink-0",
                                viewMode === "grid"
                                  ? "aspect-square mb-3 w-full"
                                  : "h-10 w-10 ml-8",
                              )}
                            >
                              {media?.thumbnail ? (
                                ...
                                />
                              ) : media?.mime_type?.startsWith("model/") ||
                                media?.name?.toLowerCase().endsWith(".glb") ||
                                media?.name?.toLowerCase().endsWith(".gltf") ? (
                              ...
                              {file.is_favorite && viewMode === "grid" && (
                                <div className="absolute top-2 right-2 bg-background/80 backdrop-blur border border-border/50 p-1 rounded-lg">
                            ...
                            >
                              <h4
                                className={cn(
                                  ...
                                  viewMode === "grid" &&
                                    "px-1 text-center sm:text-left",
                                )}
                                title={media?.title || media?.name}
                              >
                                {media?.title || media?.name || "Unknown File"}
                              </h4>
                              {viewMode === "grid" && (
                            ...
                            {viewMode === "list" && (
                              <>
                                <span className="text-[11px] font-mono font-bold bg-muted/50 px-2 py-1 rounded text-muted-foreground w-24 text-right hidden sm:block shrink-0">
                                  {media?.human_size}
                                </span>
                                <span className="text-[11px] text-muted-foreground font-mono w-32 hidden md:block shrink-0 text-right">
                                  {new Date(
                                    file.created_at,
                                  ).toLocaleDateString()}
                                </span>
                                ...
                                >
                                  ...
                                  <MenuItem
                                    ...
                                    onClick={() =>
                                      downloadFile(
                                        file.id,
                                        file.media_details?.download_name ||
                                          file.media_details?.name ||
                                          "download",
                                      )
                                    }
                                  />
                    ...
                    {!showAllFiles &&
                      ...
                      viewMode === "grid" && (
                        <div className="mt-6 flex justify-center">
                          <Button
                            variant="outline"
                            className="w-full max-w-sm rounded-xl border-dashed border-border/60 text-muted-foreground hover:border-emerald-500/50 hover:bg-emerald-500/5 hover:text-emerald-500 transition-all font-bold tracking-wide"
                            onClick={() => setShowAllFiles(true)}
                          >
                            <ChevronDown className="h-4 w-4 mr-2" /> View
                            Remaining {displayedFiles.length - 5} Files
                          </Button>
                        </div>
                      )}
                  </>
      ...
      <Dialog open={isMoveModalOpen} onOpenChange={setIsMoveModalOpen}>
        <DialogContent
          className="sm:max-w-sm rounded-[2rem] bg-background border-border/50 shadow-2xl z-[9999]"
      ...
      >
        ...
        >
          ...
          <div className="py-4">
            ...
            <Input
              value={newName || renameTarget?.name || ""}
              onChange={(e) => setNewName(e.target.value)}
              className="h-12 rounded-xl bg-muted/30 border-border/50 focus-visible:ring-emerald-500 font-medium"
              autoFocus
            />
          </div>
          <DialogFooter className="gap-2 sm:justify-end">
            ...
            <Button
              disabled={renameMut.isPending || !newName || !renameTarget}
              onClick={() => {
                if (renameTarget && newName)
                  renameMut.mutate({ ...renameTarget, name: newName });
              }}
              className="bg-emerald-500 text-emerald-950 hover:bg-emerald-400 font-bold rounded-xl px-6"
            >
              {renameMut.isPending ? (
      ...
      >
        <DialogContent
          className="sm:max-w-5xl md:max-w-[1400px] lg:max-w-[1600px] w-[98vw] md:w-full p-0 overflow-hidden bg-background/95 backdrop-blur-2xl border-border/50 rounded-[2rem] shadow-[0_0_50px_rgba(0,0,0,0.4)] flex flex-col max-h-[95vh] z-[9999]"
          overlayClassName="z-[9998]"
        >
          ...
          {selectedFile && (
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden min-h-0 w-full">
              <div
                className={cn(
                  "flex flex-1 items-center justify-center overflow-hidden bg-muted/5 relative min-h-[40vh] w-full",
                  imageEditorActive ? "p-0" : "p-4 md:border-r md:p-6",
                )}
              >
              ...
              {!imageEditorActive ? (
                <div className="w-full md:w-80 lg:w-96 flex flex-col bg-card/40 shrink-0 overflow-y-auto scrollbar-thin border-t md:border-t-0 border-border/50 relative">
                  <Button
                    ...
                    className="absolute top-4 right-4 z-50 rounded-full h-8 w-8 hover:bg-destructive hover:text-white transition-colors"
                    title="Close Preview"
                  >
                    <X className="h-4 w-4" />
                  </Button>
                  ...
                  <div className="p-6 border-b border-border/50 min-w-0 sticky top-0 bg-card/80 backdrop-blur-xl z-10">
                    <div className="flex justify-between items-start gap-4">
                      <h3
                        className="font-bold text-lg leading-tight truncate w-full pr-8"
                        title={
                          selectedFile.media_details?.title ||
                          selectedFile.media_details?.name
                        }
                      >
                        {selectedFile.media_details?.title ||
                          selectedFile.media_details?.name}
                  ...
                  <div className="flex-1 p-6 space-y-8">
                    <div className="space-y-4">
                      ...
                      <div className="space-y-4 bg-muted/20 p-4 rounded-2xl border border-border/50">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="h-8 w-8 rounded-lg bg-background flex items-center justify-center border border-border/50 shrink-0">
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-sm font-mono truncate">
                              {selectedFile.media_details?.size?.toLocaleString()}{" "}
                              bytes
                            </p>
                          </div>
                        </div>
                    ...
                    {selectedFile.media_details?.mime_type?.startsWith(
                      "video/",
                    ) &&
                      hasVideoPlayer && (
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2 min-w-0">
                            {selectedFile.media_details?.subtitles?.map(
                              (sub: FileManagerSubtitle, i: number) => (
                                <div
                                  key={i}
                                  className="flex justify-between items-center bg-muted/20 p-2.5 rounded-xl border border-border/50 text-xs min-w-0 group hover:border-emerald-500/30 transition-colors"
                                >
                                  <span className="font-medium truncate mr-2 flex-1">
                                    {sub.label}
                                  </span>
                                      ...
                                      <AlertDialog>
                                        ...
                                        <AlertDialogContent className="rounded-[2rem] border-border/50 bg-background/95 backdrop-blur-xl shadow-2xl">
                                          ...
                                          <AlertDialogFooter>
                                            ...
                                            <AlertDialogAction
                                              className="rounded-xl bg-red-500 hover:bg-red-600 text-white shadow-md font-bold"
                                              onClick={() => {
                                                if (sub.uuid)
                                                  deleteSubtitleMut.mutate(
                                                    sub.uuid,
                                                  );
                                              }}
                                            >
                          ...
                          {canManage && (
                            <>
                              <Button
                                variant="outline"
                                size="sm"
                                className="w-full border-dashed shrink-0 rounded-xl h-10 hover:bg-muted/50 hover:border-emerald-500/50 hover:text-emerald-500 transition-all"
                                onClick={() =>
                  ...
                  <div className="p-6 border-t border-border/50 bg-background/40 space-y-3 shrink-0 mt-auto sticky bottom-0 z-10 backdrop-blur-xl">
                    <Button
                      ...
                      onClick={() =>
                        downloadFile(
                          selectedFile.id,
                          selectedFile.media_details?.download_name ||
                            selectedFile.media_details?.name ||
                            "download",
                        )
                      }
                        ...
                        {downloadPhase === "downloading" &&
                          downloadTotalBytes > 0 && (
                            <p className="text-xs font-medium text-muted-foreground">
                              {formatBytes(downloadedBytes, 1)} of{" "}
                              {formatBytes(downloadTotalBytes, 1)} downloaded
                            </p>
                          )}
                        <Progress
                          ...
                          aria-label={
                            downloadPhase === "downloading"
                              ...
                              : "Secure download preparation progress"
                          }
                    ...
                    {canManage && (
                      <Button
                        variant="outline"
                        className={cn(
                          "w-full rounded-xl h-11 transition-all font-bold border-border/50 hover:bg-muted",
                      ...
                      >
                        <Star
                          className={cn(
                            ...
                            selectedFile.is_favorite && "fill-yellow-500",
                          )}
                        />{" "}
                        <span className="truncate">
                          {selectedFile.is_favorite ? "Unfavorite" : "Favorite"}
                        </span>
                      </Button>
                    )}
                  </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          setIsSubtitleModalOpen(open);
          if (!open) setSubtitleFile(null);
        }}
      >
        <DialogContent
          className="sm:max-w-md w-11/12 rounded-[2rem] bg-background border-border/50 shadow-2xl p-0 overflow-hidden box-border z-[9999]"
          overlayClassName="z-[9998]"
        >
          ...
          <div className="p-6 space-y-5 overflow-y-auto scrollbar-thin min-w-0">
            <div className="space-y-1.5 min-w-0">
              ...
              <div className="flex items-center bg-muted/50 border border-border/50 h-10 rounded-xl px-3 text-sm font-mono min-w-0 w-full overflow-hidden">
                <FileText className="h-4 w-4 mr-2 shrink-0 text-emerald-500" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
              ...
              <div className="space-y-1.5 min-w-0">
                ...
                <Input
                  ...
                  className="bg-background border-border/50 h-10 rounded-xl text-sm min-w-0 w-full"
                />
              </div>
            </div>
          </div>
          <DialogFooter className="p-6 pt-0 flex gap-2 sm:justify-end shrink-0 bg-background">
            <Button
              type="button"
              variant="outline"
            ...
            <Button
              ...
              disabled={uploadSubtitleMut.isPending || !subtitleFile}
              className="rounded-xl bg-emerald-500 text-emerald-950 font-bold px-6 shadow-md hover:bg-emerald-400 flex-1 sm:flex-none"
            >
              {uploadSubtitleMut.isPending ? (
                <>
      ...
      <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
        <DialogContent
          className="sm:max-w-md rounded-3xl bg-background border-border/50 overflow-hidden flex flex-col max-h-[90vh] shadow-2xl z-[9999]"
          overlayClassName="z-[9998]"
        >
          <DialogHeader className="shrink-0 border-b border-border/50 pb-4">
            <DialogTitle className="flex items-center gap-2">
              <UploadCloud className="h-5 w-5 text-emerald-500" /> Upload File
            </DialogTitle>
          </DialogHeader>
          <form
            id="upload-file-form"
            ...
            <div className="space-y-5 py-4 px-1">
              <div className="grid grid-cols-2 gap-4 min-w-0">
                <div className="space-y-2 min-w-0">
                  ...
                  <select
                    ...
                    className="w-full bg-muted/30 border border-border/50 h-10 rounded-xl text-sm px-3 focus:ring-2 focus:ring-emerald-500 outline-none truncate font-medium"
                  >
                    <option value="root">Root Directory</option>
                    {currentFolderId && (
                      <option value={currentFolderId.toString()}>
                ...
                <div className="space-y-2 min-w-0">
                  ...
                  <Input
                    ...
                    onChange={(e) => setUploadBaseName(e.target.value)}
                    placeholder="Optional"
                    className="bg-muted/50 border-border/50 h-10 rounded-xl text-sm min-w-0"
                  />
                </div>
              </div>
              <div className="pt-2 min-w-0 w-full">
                <div
                  ...
                  onClick={() =>
                    !uploadFileMut.isPending && fileInputRef.current?.click()
                  }
                  className={cn(
                    "border-2 border-dashed rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-all overflow-hidden w-full relative min-h-[160px]",
                    isDragging
                      ? "border-emerald-500 bg-emerald-500/10 scale-[1.02]"
                      : "border-border/50 bg-muted/20",
                    !uploadFileMut.isPending &&
                      "cursor-pointer hover:bg-muted/40 hover:border-emerald-500/50",
                  )}
                >
                  <input
                    type="file"
                  ...
                  />
                  {uploadFile ? (
                    <div className="flex flex-col items-center w-full min-w-0 overflow-hidden relative z-10">
                      <div className="relative">
                        <FileIcon
                          className={cn(
                            "h-12 w-12 text-emerald-500 mb-3 shrink-0 transition-transform duration-500",
                            uploadProgress > 0 && "scale-110 animate-bounce",
                          )}
                        />
                      </div>
                      ...
                      {uploadProgress > 0 && (
                        <div className="w-full mt-6 space-y-2 px-2 shrink-0 animate-in fade-in zoom-in duration-300">
                          ...
                          <div className="relative h-3 w-full bg-background rounded-full overflow-hidden shadow-inner border border-border/50">
                            <div
                              className="absolute top-0 left-0 h-full bg-gradient-to-r from-emerald-600 via-emerald-400 to-emerald-300 rounded-full transition-all duration-300 ease-out shadow-[0_0_15px_rgba(52,211,153,0.8)]"
                              style={{ width: `${uploadProgress}%` }}
                            >
                              <div className="absolute inset-0 bg-white/20 w-full h-full animate-[shimmer_2s_infinite] bg-[length:200%_100%]" />
                            </div>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex flex-col items-center opacity-70 group-hover:opacity-100 transition-opacity">
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        {acceptedFileDescription
                          ? `Accepted: ${acceptedFileDescription}`
                ...
                <div className="space-y-2 pt-4 border-t border-border/50 overflow-hidden min-w-0">
                  ...
                  <div className="flex items-center gap-3 w-full min-w-0">
                    <Button
                      type="button"
                      variant="outline"
                      className="h-10 border-dashed shrink-0 rounded-xl"
                    ...
                    <span
                      className="text-xs font-mono text-muted-foreground truncate flex-1 min-w-0"
                      title={
                        customThumbnail ? customThumbnail.name : "None selected"
                      }
                    >
                      {customThumbnail ? customThumbnail.name : "None selected"}
                    </span>
                    <input
                      ...
                      ref={thumbInputRef}
                      onChange={(e) =>
                        e.target.files && setCustomThumbnail(e.target.files[0])
                      }
                    />
                  </div>
                </div>
          ...
          <DialogFooter className="border-t border-border/40 p-4 shrink-0 bg-muted/10">
            <Button
              type="button"
              variant="ghost"
            ...
            <Button
              ...
              className="rounded-xl bg-emerald-500 hover:bg-emerald-400 text-emerald-950 font-bold shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:shadow-[0_0_20px_rgba(16,185,129,0.5)] transition-all flex-1 sm:flex-none px-8"
            >
              {uploadFileMut.isPending ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" /> Uploading...
      ...
      <Dialog open={isAddToPlaylistOpen} onOpenChange={setIsAddToPlaylistOpen}>
        <DialogContent
          className="sm:max-w-md rounded-[3rem] bg-background/80 backdrop-blur-3xl border-border/40 shadow-2xl p-0 overflow-hidden z-[9999]"
          overlayClassName="z-[9998]"
        >
          <div className="p-8 border-b border-border/40 flex items-center justify-between bg-pink-500/5">
            <div>
              <DialogTitle className="flex items-center gap-2 text-2xl font-black tracking-tight">
                <Music className="h-6 w-6 text-pink-500" /> Add to Playlist
              </DialogTitle>
              <DialogDescription className="text-sm font-medium mt-1">
                Select a destination for your sound.
              </DialogDescription>
            </div>
          </div>
            ...
            {playlists.length === 0 && (
              <div className="text-center py-12 px-6">
                ...
                <p className="text-xs text-muted-foreground font-medium">
                  Create your first playlist below to start organizing.
                </p>
              </div>
            )}
          </div>
          <DialogFooter className="p-8 pt-6 border-t border-border/40 bg-pink-500/5 flex flex-col gap-4">
            <div className="relative w-full">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (input.value) {
                    createPlaylistMut.mutate(input.value);
                    input.value = "";
                  }
                }}
                disabled={createPlaylistMut.isPending}
              >
                {createPlaylistMut.isPending ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
            ...
            <div className="flex gap-3 w-full">
              <Button
                type="button"
                variant="ghost"
                className="rounded-2xl h-11 flex-1 font-bold hover:bg-pink-500/5"
...
function MetricCard({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "border p-3.5 rounded-2xl flex flex-col gap-2 min-w-0 transition-all cursor-pointer group",
        color,
        isActive
          ? "ring-2 ring-emerald-500 ring-offset-2 ring-offset-background scale-[1.02] shadow-md border-emerald-500/50"
          : "hover:bg-muted/50 hover:scale-[1.01]",
      )}
    >
      <div className="flex items-center justify-between w-full">
        <div className="bg-background/80 p-2 rounded-xl border border-border/50 shadow-sm shrink-0">
          {icon}
        </div>
        <Badge
          variant="outline"
